  ...
  try {
    ...
    // error retry message if book title, author, or userID does not match
      if (!title || !author || !username) {
        res.status(400).json({ error: "Title, author or username is missing in the request." }
        };
      return;
  }

  // inserting the book into a user's wishlist
  await db.collection("wishlists").updateOne({ username }, { $push: { books: { title, author, publicationYear } } } );
